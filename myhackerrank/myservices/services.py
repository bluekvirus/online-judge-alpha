...
import requests
import json
from myservices.models import Submission, Problem, Interview
...
@methods('POST')
@service
def postSubmission(req, res, *args, **kwargs):
	f = open('/app/hrank.txt', "r")
	contents = json.load(f)
	f.close()
	code = req.json['code']
	language = req.json['language']
	problem = req.json['problem-id'] #does this become problem id then??
	uid = req.json['user_id']
	...
	if query.problems.filter(id=problem).exists():
		problem_name = Problem.objects.filter(id=problem)
		response = requests.post('https://www.hackerrank.com/rest/contests/master/challenges/' + problem_name + '/submissions', json=j, headers={'Content-Type': 'application/json', 'X-CSRF-Token': contents['csrf']}, cookies={'_hrank_session': contents['hrank']})
		j = response.json()
		...
		if(status == "Processing" or status == "Queued"):
			...
			producer.send('hrank_results', {'sid': submit_id, 'problem': problem_name})
			b = Submission(user_id=uid, submit_id=submit_id, result=status, problem_id=problem)
			b.save()
...
def getResults(req, res, *args, **kwargs):
	if(len(req.params) == 1):
		uid = req.params['uid']
		query = Submission.objects.filter(user_id=uid) 
		qjson = serializers.serialize('json', query)
		j = json.loads(qjson)
		return res.json(j, safe=False)
	elif(len(req.params) == 2):
		uid = req.params['uid']
		problem = req.params['problem'] #this is the problem id
		query = Submission.objects.filter(user_id=uid).filter(problem_id=problem) #have to add some additional query because now the submission table should have problem id not name
		qjson = serializers.serialize('json', query)
		j = json.loads(qjson)
		return res.json(j, safe=False)
...
def interview(req, res, hashstr, *args, **kwargs):
	...
	if not query: #or query.exist()
		...
		if not started:
			...
			problem_list = query.problems.all()
